...
public class FTPUtils {
	...
	//@PostConstruct
	public boolean init() {
		ftpProperties.setUsername(env.getProperty("ftp.userName"));
		ftpProperties.setPassword(env.getProperty("ftp.passWord"));
		ftpProperties.setBaseUrl(env.getProperty("ftp.baseUrl"));
		ftpProperties.setPort(Integer.valueOf(env.getProperty("ftp.port")));
		ftpProperties.setHost(env.getProperty("ftp.host"));
	...
	public static boolean uploadFile(InputStream input) {
		boolean result = false;
		try {
			getFTPClient();
			ftpClient.enterLocalPassiveMode();
			result = ftpClient.storeUniqueFile(input);
			input.close();
			ftpClient.disconnect();
		} catch (Exception e) {
			e.printStackTrace();
